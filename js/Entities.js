  ...
  self.performSpecialAttack = function () {
    if (self.attackCounter > 50) {
      ...
      /*
			...
			*/
      Bullet.generate(self, self.aimAngle - 20);
...
Enemy.randomlyGenerate = function () {
  ...
  var width = 64 * 1.5;
  var id = Math.random();
  if (Math.random() < 0.5) Enemy(id, x, y, width, height, Img.bat, 2, 0.5);
  else Enemy(id, x, y, width, height, Img.bee, 1, 0.5);
};
Enemy.generateBoss = function () {
  //Math.random() returns a number between 0 and 1
  var x = Math.random() * Maps.current.width;
  var y = Math.random() * Maps.current.height;
  var height = 128 * 1.5;
  var width = 128 * 1.5;
  var id = "Boss";
  if (Math.random() < 0.5) Enemy(id, x, y, width, height, Img.bat, 200, 0.75);
  else Enemy(id, x, y, width, height, Img.bee, 200, 0.75);
};
...
Upgrade.update = function () {
  ...
  for (var key in Upgrade.list) {
    ...
    if (isColliding) {
      if (player.hp <= 15) {
        if (Upgrade.list[key].category === "hp_regen") player.hp += 2;
      }
